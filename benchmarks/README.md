...
# `go-ruby-digest` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-digest` library**
...
reference runtime's own `digest`?*

## Layout

- `go/`          — self-contained Go driver; `go.mod` pins the published library.
- `ruby/digest.rb`  — the equivalent workload; `ruby/_harness.rb` is the shared timer.
