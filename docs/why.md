...
`go-ruby-digest/digest` reimplements Ruby's Digest message-digest suite in **pure Go, with cgo disabled**. The
...
- any Go program can import `github.com/go-ruby-digest/digest` directly, with no Ruby runtime;
